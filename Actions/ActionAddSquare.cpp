...
void ActionAddSquare::Execute() 
{
	...
	if (P1.y < UI.StatusBarHeight || P1.y > UI.height - UI.StatusBarHeight || P1.x < UI.MenuItemWidth)
	{
	...
	if (P2.y < UI.StatusBarHeight || P2.y > UI.height - UI.StatusBarHeight || P2.x < UI.MenuItemWidth)
	{
		bool flag = false; //false as long as the click is in wrong place
		pGUI->PrintMessage("Please, Choose a valid Point");
		while (!flag)
